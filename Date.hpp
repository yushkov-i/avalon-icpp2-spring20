...
/*----------homework-04----------*/
	...
	*/
	Season getSeason(Date date) {
		if (static_cast<int>(date.month) < 3 && static_cast<int>(date.month) > 0|| static_cast<int>(date.month) == 12) {
		...
		else if (static_cast<int>(date.month) > 8 && static_cast<int>(date.month) < 12) {
	...
	*/
	Date operator + (const Date date, const TimeDelta delta) {
		...
		a = countJND(date);
		int deltday = delta.delta;
		a.delta = a.delta + 32044 + deltday;
	...
	TimeDelta operator + (const TimeDelta lhs, const TimeDelta rhs) {
		TimeDelta res;
		res.delta = (static_cast<int>(lhs.delta) + static_cast<int>(rhs.delta));
		return res;
	}
