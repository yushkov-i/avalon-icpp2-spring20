...
namespace ext
{
	...
	*/
	void swap(TimeDelta& lhs, TimeDelta& rhs) {
		TimeDelta temp;
		temp.delta = 0;     //мой компьютер выводит, что "temp неинициализирована" и не запускал, причем только в этой функции...странно!
		lhs = rhs;			//у меня часто похожие глупости выходят, так и не понимаю, почему он не хочет норм. работать
		rhs = temp;
	}
